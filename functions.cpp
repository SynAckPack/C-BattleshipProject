...
bool checkHit(char x, char y, const Ship &s) {		//Checks the coordinates of the ship versus the user input
	...
	int f = 0;
	for (f = 0; f < s.size; f++) {
		if (s.shipRow[f] == shotRow && s.shipCol[f] == shotCol) return true;
	}
	return false;
}
...
void gameDisplay(char x = ' ', char y = ' ') {		//Displays edited game board
	...
	for (i = 1; i < rows; ++i) {						//Begins checking
		for (j = 1; j < columns; ++j) {
			...
			if (i == 1) {
				board[i][j] = rowLabel;
				if (rowLabel == '/') board[i][j] = ' ';
				rowLabel = rowLabel + 1;
			}
			if (j == 1) {
				board[i][j] = colLabel;
				if (colLabel == '@') board[i][j] = ' ';
				colLabel = colLabel + 1;
			}
	...
	if (battleshipHit == true) {					//Increments counter if the battleship is hit. Once counter reaches 4 hits, the battleship is sunk and the game ends.
		++::battleshipHitCount;
		if (::battleshipHitCount == 4) battleship.status = "destroyed!";
	}
	shotAttempt();
}
...
char shotAttempt() {
	...
	if (endGame == false) {
		...
		cin >> row;
		if (islower(row)) row = toupper(row);			//Handles if lowercase letters are used instead of uppercase letters.	
		cin >> col;
		try {
			if (col < 48 || col > 57) throw(errorMessage);
		}
		catch (...) {
...
void blankBoardDisplay() {							//Displays initial blank game board.
	for (int a = 1; a < rows; ++a) {
		for (int b = 1; b < columns; ++b) {
			if (a == 1 && b == 1) blankBoard[a][b] = ' ';
			if (a == 1) {
				blankBoard[a][b] = rowLabel2;
				if (rowLabel2 == '/') blankBoard[a][b] = ' ';
				rowLabel2 = rowLabel2 + 1;
			}
			if (b == 1) {
				blankBoard[a][b] = colLabel2;
				if (colLabel2 == '@') blankBoard[a][b] = ' ';
